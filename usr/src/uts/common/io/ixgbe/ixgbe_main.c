...
ixgbe_free_rings(ixgbe_t *ixgbe)
{
	...
	for (uint_t i = 0; i < ixgbe->num_rx_groups; i++) {
		...
		for (ixgbe_vlan_t *vlp = list_head(&rx_group->vlans);
		     vlp != NULL;
...
ixgbe_init_vlan(ixgbe_t *ixgbe)
{
	...
	for (uint_t i = 0; i < ixgbe->num_rx_groups; i++) {
		...
		for (ixgbe_vlan_t *vlp = list_head(&rxg->vlans); vlp != NULL;
			...
			/*
			 * In order for this software state to exist
			 * the hardware must have accepted it at some
			 * point. Therefore, it should still accept it
			 * after a chip reset.
			 */
...
ixgbe_addvlan(mac_group_driver_t gdriver, uint16_t vid)
{
	...
	if (ret == IXGBE_ERR_PARAM)
		...
		panic("Unexpected return from ixgbe_set_vfta(): %d, file: %s"
		    " line: %d", ret, __FILE__, __LINE__);
...
ixgbe_remvlan(mac_group_driver_t gdriver, uint16_t vid)
{
	...
	if (ret != IXGBE_SUCCESS) {
		/* This is the only possible error here. */
		VERIFY3S(ret, ==, IXGBE_ERR_PARAM);
		mutex_exit(&ixgbe->gen_lock);
		return (EINVAL);
	}
